...
{
    public class UP11APageHandler : PageHandler
    {
        #region " Overrides... "
        public override void OnRejectOffer()
        {
            var UP5active = Form["Up11Active"] ?? string.Empty;
            var UP6active = Form["Up6Active"] ?? string.Empty;
            var UP5quantity = Form["Up11hiddenQty"] ?? "0";
            ...
            {
                OrderManager.SetProductQuantity("UP11", Int32.Parse(UP5quantity));
            }
