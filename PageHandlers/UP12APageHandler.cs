...
{
    public class UP12APageHandler : PageHandler
    {
        #region " Overrides... "
        public override void OnRejectOffer()
        {
            var UP5active = Form["Up12Active"] ?? string.Empty;
            var UP6active = Form["Up11Active"] ?? string.Empty;
            var UP5quantity = Form["Up12hiddenQty"] ?? "0";
            var UP6quantity = Form["Up11hiddenQty"] ?? "0";
            bool isUp5Active;
            bool isUp6Active;

            if (Boolean.TryParse(UP5active, out isUp5Active) && isUp5Active)
            {
                OrderManager.SetProductQuantity("UP12", Int32.Parse(UP5quantity));
            }
            if (Boolean.TryParse(UP6active, out isUp6Active) && isUp6Active)
            {
                OrderManager.SetProductQuantity("UP11", Int32.Parse(UP6quantity));
            }
