...
<%
    ...
    var linkFAQ = isFAQ ? hashFAQ : String.Format("FAQ{0}", DtmContext.ApplicationExtension);
        ...
        <picture class=""contain contain--logo"">
            <source srcset=""/images/logo--new.webp"" media=""(min-width: 601px)"" type=""image/webp"">
			<img src=""/images/logo--new.png"" alt=""{0}"">
        </picture>", productName);
        var navLogo = logo;

        if (isStartPage)
        {
            var logoContainer = @"<a href=""{0}"" id=""{2}"" class=""{3}"">{1}</a>";
        ...
        {
            var logoContainer = @"<div class=""{1}"">{0}</a>";
