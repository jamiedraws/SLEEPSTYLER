...
<%
    ...
    if (preload)
    {
        ...
        {
            "/css/16.0000/default/shell.css"
        };

        if (isIndex)
        {
            styles.Add("/css/16.0000/default/index.crp.css");
        }

        if (isSubPage) {
            styles.Add("/css/16.0000/default/landing-page.css");
        }

        if (isFAQ) {
            styles.Add("/css/16.0000/default/faq.css");
        }

        if (isReviews) {
            styles.Add("/css/16.0000/default/reviews.css");
        }

        if (isPaymentPage) {
            styles.Add("/css/16.0000/default/checkout.css");
        }

        if (isUpsell) {
            styles.Add("/css/16.0000/default/upsell.css");
        }

        if (isConfirmation) {
            styles.Add("/css/16.0000/default/confirmation.css");
        }

        styles.Add("https://use.typekit.net/vzk6ibt.css");
    ...
    if (!preload && defer)
    {
        ...
        if (isIndex) {
            ...
            scripts.Add("/js/16.0000/index.js");
        }

        if (isLandingPage) {
            scripts.Add("/js/16.0000/landing-page.js");                
            scripts.Add("/shared/js/common.js?v=" + Dtm.Framework.ClientSites.Web.DtmContext.ApplicationVersion.ToString() + "&language=" + Dtm.Framework.ClientSites.SettingsManager.ContextSettings["Language.LanguageType", "English"] + "&cb=1");
        }
        ...
        if (isPaymentPage) {
            scripts.Add("/js/16.0000/checkout.js");
        }
