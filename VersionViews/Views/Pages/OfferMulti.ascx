...
<% 
    ...
    if (hasProduct)
    {
        var price = Convert.ToDecimal(SettingsManager.ContextSettings["Affirm.AffirmEstimatePaymentAmount"]);
        ...
        var centsLabel = hasCents ? cents.ToString() : "/mo";

        var standfirst = "For as low as";
        var label = String.Format("Payments as low as {0}/month with Affirm", price.ToString("C2"));
