(function (global) {
    if ("app" in global) {
        ...
        addEventListener("SessionLoaded", function () {
            ...
            let hasV6Product = false;

            for (let i = 0; i < cart.cartItems.length; i++) {
                if (!hasV6Product) {
                    hasV6Product = v6Items.indexOf(cart.cartItems[i]) > -1;
                }
            }
            ...
            if (
                ...
                let pageCode = (window["PageCode"] || "").toUpperCase();
                const mainItemDefaultDictionary = {
                    BESTVALUE: "product3",
                    TVOFFER: "product1"
                };
                const filterMainItemDefault = Object.keys(
                    mainItemDefaultDictionary
                ).filter(function (key) {
                    return key === pageCode;
                });
                const hasDefaultMainItem = filterMainItemDefault.length > 0;
                const id = hasDefaultMainItem
                    ? mainItemDefaultDictionary[filterMainItemDefault.pop()]
                    : "product2";
